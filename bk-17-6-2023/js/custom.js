...
//     arrows: false
// });
let time, previousSecond, countDownDate = new Date().getTime() + 1000 * 60 * 60 * 1.5,
...
function updateTime() {
    ...
    setTimeout(() => {
        if (previousSecond !== time[time.length - 1])
            time.forEach((num, i) => doSplitflap(splitFlaps[i], num)), previousSecond = time[time.length - 1];
    }, 100);
}
...
function separateIntoSingleDigits(time) {
    return time.map(num => ("0" + num.toString()).slice(-2)).join("").split("");
}
...
counters.forEach((counter) => {
    ...
    // const myTimeout = setTimeout(updateCounter(), 5000);
    updateCounter();
});
